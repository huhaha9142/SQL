...
SELECT first_name, 
    ...
    department_name
FROM employees emp, departments dept
WHERE emp.department_id = dept.department_id (+);   -- (+) 쪽의 NULL출력
...
------------
-- 자기 자신과 JOIN
-- 자기 자신을 두 번 이상 호출 -> alias(별칭)을 사용할 수 밖에 없는 JOIN
...
-- count: 갯수 세기
SELECT count(*) FROM employees; --  특정 컬럼이 아닌 레코드의 개수를 센다

SELECT count(commission_pct) FROM employees;    -- 해당 컬럼이 null이 아닌 개수
